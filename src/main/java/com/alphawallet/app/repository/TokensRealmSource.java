...
public class TokensRealmSource implements TokenLocalSource {
    ...
    private TokenTicker convertRealmTicker(RealmTokenTicker rawItem) {
        ...
        if (rawItem != null) {
            ...
            if (rawItem.getChain() == WOOOO.CHAIN_ID) {
                double wooFiat = Double.parseDouble(price) * WOOOO.DOLLAR_RATE;
                price = Double.toString(wooFiat);

            }

            tokenTicker = new TokenTicker(price, rawItem.getPercentChange24h(), currencySymbol, rawItem.getImage(), rawItem.getUpdatedTime());
        }
    ...
    private Single<Pair<Double, Double>> calculateWalletValue(TokenCardMeta[] metas, Wallet wallet) {
        return Single.fromCallable(() -> {
            ...
            for (TokenCardMeta meta : metas) {
                ...
                {
                    Log.d("calculateWalletValue", "TokenTicker Price : " + ticker.priceSymbol);
